...
function userPromptPasswordLength() {
  let lengthOfPassword = prompt(
    "Enter the number of characters you would like your password between 10 to 64 characters."
  );

  if (lengthOfPassword == null) {
    //If the user clicks cancel then the window will reload. //
    location.reload();
  } else if (lengthOfPassword == "") {
    //If the user clicks ok and does not enter a number then they will get an alert advising them to.//
...
function getPasswordOptions() {
  ...
  let specialPrompt = window.confirm(
    "Do you want special characters? Click ok(Yes) or cancel(No)."
  );
  if (specialPrompt) {
  ...
  let numericPrompt = window.confirm(
    "Do you want numbers? Click ok(Yes) or cancel(No)."
  );
  if (numericPrompt) {
  ...
  let lowerCasePrompt = window.confirm(
    "Do you want lowercase letters? Click ok(Yes) or cancel(No)."
  );
  if (lowerCasePrompt) {
  ...
  let upperCasePrompt = window.confirm(
    "Do you want uppercase letters? Click ok(Yes) or cancel(No)."
  );
  if (upperCasePrompt) {
...
function writePassword() {
  ...
  if (newPasswordString != "") {
    //This conditional statement checks whether the newPasswordString is empty when the user clicks Generate Password, it's not then it clears it before providing the user with a new password.
    newPasswordString = "";
    arrayPasswordInput = [];
...
function copyPassword() {
  ...
  if (generatePassword != "") {
    //This function when executed selects the whole text area and copies the password text making it easier to copy on mobile devices.
