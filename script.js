...
let newPasswordString = ''; //This is an emtpy variable which will store a new string that is generated by the getRandom function.//


// This function prompts the user to enter a password length from 10-64 and it will repeat if the user does not meet this condition, once the condition is met the value is stored in the variable userNumberInput.//
function userPasswordLength () {
  ...
  if (lengthOfPassword < 9 || lengthOfPassword > 64) {
    ...
    userPasswordLength(); //Calling the function again will prompt the user to input a number that meets the condition.//
  }
  else {
...
// This function prompts the user for password options (special characters etc) and if conditions are met then the corresponding array is concatenated into the variable arrayPasswordInput.//
...
// This function executes the getPasswordOptions first in order to be able to have an array that an be looped through in the getRandom function, it then returns the string that was generated by the getRandom function to be used as the passwordText.value in the writePassword function.
